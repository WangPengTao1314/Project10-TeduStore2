  ...
  <div class="rightsidebar_box rt">
   <div class="rs_header">
    <span class="rs_header_active"><a
     href="${pageContext.request.contextPath}/showPersonInfo.do">我的信息</a></span>
    <span><a
     href="${pageContext.request.contextPath}/showPassword.do">安全管理</a></span>
   </div>

   <!--个人信息具体内容 -->
   <div class="rs_content">
    <!--头像-->
    <div class="rs_content_headPortrait" style="height:92px;" >
     <span class="same">我的头像：</span>
          <c:if test="${user.image==null}">
             <img src="${pageContext.request.contextPath}/images/personage/touxiang.png"
      ...
      <input type="file" name="file" 
          value="" id="iconPic" onchange="getImage()"> 
       </form>
     <!--  <span 
    ...
    <form id="personInfor">
     <div class="rs_content_username">
      ...
      <input id="username" name="username" class="ed_username" value=""
     ...
     <div class="rs_content_sex">
      ...
      <span class="same">性别：</span> <input type="radio" name="gender"
       ...
       <c:if test="${user.gender==0}">checked</c:if>>男 <input
       type="radio" name="gender" id="gender" value="${user.gender}"
       <c:if test="${user.gender==1}">checked</c:if>>女

     </div>
     <!--绑定电话-->
     <div class="rs_content_tel">
      <span class="same">绑定电话：</span> <input id="phone" name="phone"
       type="text" value="${user.phone}" />
     </div>
     <!--绑定邮箱-->
     <div class="rs_content_mail">
      <span class="same">绑定邮箱：</span> <input id="email" name="email"
       class="ed_email" value="" style="display: none;" /> <span
       class="rs_mail">${user.email}</span> <span
       class="same_click change_mail">更改邮箱</span>
     </div>
     <!--保存按钮-->
     <div class="save">
      <a href="#" style="color: #fff" onclick="btnUpdate()"> 保存更改 </a>
     </div>
    </form>
   </div>
  </div>
...
<script type="text/javascript">
	...
	function btnUpdate() {
		$
				.ajax({
					"url" : "${pageContext.request.contextPath}/changePersonInfor.do",
					"data" : $("#personInfor").serialize(),
					"dataType" : "json",
					"type" : "post",
					"success" : function(obj) {
						if (obj.state == 1) {
							location.href = "${pageContext.request.contextPath}/showPersonInfo.do"
						} else {
